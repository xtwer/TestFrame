...
class Email:
    ...
    def _message_init(self):
        ...
        if self.context:
            self.message.attach(MIMEText(self.context,'html','utf-8'))#邮件正文
    ...
    def _attach(self,file):
        if os.path.isfile(file) and os.path.getsize(file) <= _FILESIZE * 1024 * 1024:
            ...
            attach.add_header('Countent-Disposition','attachment',filename=os.path.basename(file))
    ...
    def send_mail(self):
        ...
        result = True
        try:
            s.login(self._smtp_cfg['user'],self._smtp_cfg['password'])
